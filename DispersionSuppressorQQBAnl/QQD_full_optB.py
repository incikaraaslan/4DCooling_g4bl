...
def run_all(particle_file=None, do_plots=True, do_optimize=True, ds=1e-3):
    ...
    optics_template = [
        ("quad", 0.2, 3.0000, 0.109758),  # L=0.03 m, B_pole=0.02965 T, r=0.1 m
        ("drift", 0.01),
        ("quad", 0.2, -2.607288, 0.109758), # L=0.025 m, B_pole=-0.014825 T, r=0.1 m
        ("drift", 0.02),
        ("dipole", 0.0320, 1.5, 0.1, 0.1),  # L=0.03 m, B=1.5 T, width=0.1 m , height=0.1 m
    ]
    ...
    if do_optimize:
        ...
        res = differential_evolution(
            func=lambda x: region_cost_br(x, optics_template, x_params, y_params, Brho, ds=ds, reg=1e-6),
            bounds=bounds,
            maxiter=800,
            popsize=8,
            tol=1e-8,
        ...
        res = minimize(
            lambda x: region_cost_br(x, optics_template, x_params, y_params, Brho, ds=ds, reg=1e-6),
            x0,
            method="Nelder-Mead",
            bounds=bounds,
            options={"ftol": 1e-12},
