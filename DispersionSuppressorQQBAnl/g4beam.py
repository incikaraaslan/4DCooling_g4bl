...
import pandas as pd
from tqdm import *
print("LOADED g4beam FROM:", __file__)
...
def run_g4beam(df, filename, **kwargs):
    ...
    out_base = f"out_{ident}"             # OUTPUT: basename ONLY
    out_filename = f"{out_base}.txt"      # what G4BL actually writes
    write_trackfile(df, in_filename)
    command = ["g4bl", filename, f"beamfile={in_filename}", f"outname={out_base}"] + [x + "=" + str(y) for x, y in kwargs.items()]
    
    try:
        subprocess.run(command, stdout=subprocess.DEVNULL)
        print("Files after G4BL:", [f for f in os.listdir(".") if f.endswith(".txt")])
    except Exception:
